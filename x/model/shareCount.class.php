...
class shareCount {
	...
	function get_tweets() { 
		...
		return isset($json['count'])?$this->convertcount(intval($json['count'])):0;
	}
	...
	function get_fb() {
		...
		$json = json_decode($json_string, true);
		return isset($json[0]['total_count'])?$this->convertcount(intval($json[0]['total_count'])):0;*/
		$json_string = $this->file_get_contents_curl('http://graph.facebook.com/?id='.$this->url);
		$json = json_decode($json_string, true);
		return isset($json['shares'])?$this->convertcount(intval($json['shares'])):0;
	}
	...
	function get_plusones()  {
		...
		return isset($json[0]['result']['metadata']['globalCounts']['count'])?$this->convertcount(intval( $json[0]['result']['metadata']['globalCounts']['count'] )):0;
	}
	...
	function get_stumble() {
		...
		$json = json_decode($json_string, true);
		return isset($json['result']['views'])?$this->convertcount(intval($json['result']['views'])):0;
	}

	function get_delicious() {
		$json_string = $this->file_get_contents_curl('http://feeds.delicious.com/v2/json/urlinfo/data?url='.$this->url);
		$json = json_decode($json_string, true);
		return isset($json[0]['total_posts'])?$this->convertcount(intval($json[0]['total_posts'])):0;
	}
	...
	function convertcount($postresultscount){
		$k = 1000;
		if ($postresultscount >= $k) { 
		  $echoxcount = round($postresultscount/$k);

		  if ($echoxcount >= $k) {
		  	$echoxcount = number_format($echoxcount, 0).'K';
		  	#$echoxcount = $echoxcount.'K';
		  } else {
		  	$echoxcount = $echoxcount.'K';
		  }
