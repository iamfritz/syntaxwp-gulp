...
class Personalize_Login_Plugin {
    ...
    function set_variable(){
        ...
        $this->lost_pass_url 	= home_url( 'lost-password' );
        $this->reset_pass_url 	= home_url( 'reset-password' );
        $this->register_url 	= home_url( 'register' );
	...
	public function redirect_after_logout() {
		$redirect_url = $this->login_url.'?logged_out=true';
		...
		if ( 'GET' == $_SERVER['REQUEST_METHOD'] ) {
			...
			if ( ! $user || is_wp_error( $user ) ) {
				if ( $user && $user->get_error_code() === 'expired_key' ) {
					wp_redirect( $this->login_url.'?login=expiredkey' );
				} else {
					wp_redirect( $this->login_url.'?login=invalidkey' );
				}
				exit;
			}

			$redirect_url = $this->reset_pass_url;
	...
	public function do_register_user() {
		if ( 'POST' == $_SERVER['REQUEST_METHOD'] ) {
			$redirect_url = $this->register_url;
			...
			if ( ! get_option( 'users_can_register' ) ) {
				...
				$result = $this->register_user( $email, $first_name, $last_name/*, $pass1, $pass2*/ );
	...
	public function do_password_lost() {
		if ( 'POST' == $_SERVER['REQUEST_METHOD'] ) {
			...
			if ( is_wp_error( $errors ) ) {
				// Errors found
				$redirect_url = $this->lost_pass_url;
	...
	public function do_password_reset() {
		if ( 'POST' == $_SERVER['REQUEST_METHOD'] ) {
			...
			if ( isset( $_POST['pass1'] ) ) {
				...
				wp_redirect( $this->login_url.'?password=changed' );
			} else {
	 ...
	 */
	private function register_user( $email, $first_name, $last_name/*, $pass1, $pass2*/) {
		$errors = new WP_Error();
		...
		/*if(!$pass1 || !$pass2) {
			$errors->add( 'password_reset_empty', $this->get_error_message( 'password_reset_empty') );
			return $errors;
		} else if($pass1 != $pass2) {
...
$theme_profiler = new Personalize_Login_Plugin();
