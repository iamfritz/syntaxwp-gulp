 ...
 */
class Paypal_Ipn_For_Wordpress_Post_types {
    ...
    public static function paypal_ipn_for_wordpress_register_post_types() {
        ...
        register_post_type('paypal_ipn', apply_filters('paypal_ipn_for_wordpress_register_post_type_ipn', array(
            ...
            'rewrite' => array('slug' => 'paypal_ipn'),
            'query_var' => true,
            'menu_icon' => ASSETS_IMG . 'paypal-icon.png',
            'supports' => array('', ''),
            'has_archive' => true,
            'menu_position'       => 22,
    ...
    public static function paypal_ipn_for_wordpress_add_paypal_ipn_columns($existing_columns) {
        ...
        $columns['business'] = _x('Business', 'paypal-ipn');
        $columns['packages'] = __('Packages', 'paypal-ipn');
    ...
    public static function paypal_ipn_for_wordpress_render_paypal_ipn_columns($column) {
        ...
        switch ($column) {
            ...
            case 'first_name' :
                ...
                break;
            case 'business' :
            	$biz_id = get_post_meta($post->ID, 'post_id', true);
                echo '<a href="'.esc_url( get_permalink($biz_id) ).'" target="_blank">'.get_the_title($biz_id).'</a>';
                break;    
            case 'packages' :
                echo esc_attr(get_post_meta($post->ID, 'package_id', true));
                break;                            
            case 'mc_gross' :
...
Paypal_Ipn_For_Wordpress_Post_types::init();
